...
func main() {
	...
	graph := [][]int{{1}, {0}, {4}, {4}, {2, 3}}
	bipartite := isBipartite(graph)
	fmt.Println(bipartite)
}
/*
二分图：如果我们能将一个图的节点集合分割成两个独立的子集A和B，并使图中的每一条边的两个节点一个来自A集合，一个来自B集合，我们就将这个图称为二分图
...
func isBipartite(graph [][]int) bool {
	...
	for i, arr := range graph {
		for _, a := range arr {
			if isUnion(parent, i, a) {
				return false
			}
			union(parent, arr[0], a)
		}
	}
	return true
...
func find(parent []int, p int) int {
	for parent[p] != p {
		p, parent[p] = parent[parent[p]], parent[parent[p]]
	}
	return p
}
func isUnion(parent []int, p, q int) bool {
	return find(parent, p) == find(parent, q)
}
func union(parent []int, p, q int) {
	parent[find(parent, q)] = find(parent, p)
}
